...
Picked from `tests/fixtures/routing-eval.json` (97 real user queries, one per tm capability) for spread of `describeCapability` contract size — read-only capabilities only, each with a recorded 200 response in `tests/live/runs/tm/` so the turn-4 correction is available for every row. (`get_test_case_linked_issues`, originally selected for its mid-range size, has no successful recorded run — `status: null`/`BLOCKED` — and was swapped for `get_report`, comparable in size, which does.)

| capability | raw contract size (bytes) | query |
|---|---:|---|
| `count_binned_test_cases` | 1,485 | "how many cases are in the bin right now" |
| `list_configuration_groups` | 1,752 | "what configuration groups does the account have" |
| `get_test_case` | 2,095 | "open TC-1042 and show me everything on it" |
| `list_test_plans` | 3,015 | "what plans exist in this project" |
| `get_report` | 3,778 | "show me the configuration behind SC-12" |
| `get_test_run` | 3,265 | "pull up the details of this run" |
| `list_test_case_comments` | 3,686 | "read the comments on this case" |
| `list_test_run_test_case_comments` | 5,179 | "read the notes people left on this case inside the run" |
| `list_test_run_test_cases` | 7,409 | "which cases are in this run, and who owns each one" |
| `search_all_projects` | 10,142 | "search everywhere in the account for the word checkout" |
...
**Smallest contract — `count_binned_test_cases`** ("how many cases are in the bin right now"):
...
**Median contract — `get_report`** ("show me the configuration behind SC-12"):
...
**Largest contract — `search_all_projects`** ("search everywhere in the account for the word checkout"):
...
|---|---:|---:|---:|---:|
| `count_binned_test_cases` | 22,278 / 22,294 | 10,313 / 10,333 | 128 | +16 |
| `list_configuration_groups` | 21,167 / 21,216 | 9,956 / 10,018 | 119 | +49 |
| `get_test_case` | 27,021 / 27,381 | 13,211 / 13,661 | 136 | +360 |
| `list_test_plans` | 20,856 / 21,009 | 10,123 / 10,314 | 134 | +153 |
| `get_report` | 22,137 / 22,303 | 10,718 / 10,925 | 129 | +166 |
| `get_test_run` | 26,932 / 27,627 | 13,749 / 14,618 | 136 | +695 |
| `list_test_case_comments` | 19,647 / 20,188 | 9,510 / 10,186 | 143 | +541 |
| `list_test_run_test_case_comments` | 21,543 / 21,687 | 10,610 / 10,790 | 155 | +144 |
| `list_test_run_test_cases` | 25,438 / 25,687 | 12,893 / 13,204 | 148 | +249 |
| `search_all_projects` | 36,289 / 36,657 | 19,836 / 20,296 | 142 | +368 |
