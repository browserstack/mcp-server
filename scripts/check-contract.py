...
def drift_between(old_path, new_path):
    ...
    list_binned_test_cases regressed, and it is only visible as a diff.
    """
    _, old = load(old_path)
...
def survey(path):
    """The three findings, as data, so a baseline can be compared against them."""
    d, tm = load(path)
    unbacked, no2xx, empty = {}, [], []
    for c in tm['capabilities']:
        ret = c.get('returns') or []
        r = (c.get('responses') or {})
        node = r.get('200') or r.get('201') or r.get('202')
        if not any(k in r for k in ('200', '201', '202', '204')):
            no2xx.append(c['name']); continue
        if isinstance(node, dict) and node.get('schema') == {'type': 'object'}:
            empty.append(c['name']); continue
        if not ret or not node:
            continue
        names = {n for _, n in resolve(tm, node)}
        missing = sorted(f for f in ret if f.split('.')[-1] not in names)
        if missing:
            unbacked[c['name']] = missing
    return {'index': path, 'unbacked': unbacked,
            'no_2xx': sorted(no2xx), 'empty_2xx': sorted(empty)}


def against_baseline(path, baseline_path, quiet=False):
    """
    Fail on REGRESSIONS ONLY, which is what the gate was always for.

    The unqualified check fails whenever a single unbacked field exists anywhere, and this
    index has 60 of them across 16 capabilities — every one already known, most of them
    waiting on a product-side response schema nobody on this side can write. A gate that
    is red on every commit stops being read, and then the one commit that adds a NEW
    disagreement goes through unnoticed. The baseline records what is known, so the build
    breaks on what is new.

    `package.json` has passed `--baseline` since the gate was added; the flag was never
    implemented and the argument was silently ignored, so CI has been failing on the
    pre-existing set rather than on regressions.
    """
    import json as _json
    now = survey(path)
    base = _json.load(open(baseline_path))
    b_unbacked = base.get('unbacked') or {}

    added = {}
    for name, fields in now['unbacked'].items():
        new_fields = sorted(set(fields) - set(b_unbacked.get(name, [])))
        if new_fields:
            added[name] = new_fields
    new_no2xx = sorted(set(now['no_2xx']) - set(base.get('no_2xx') or []))
    new_empty = sorted(set(now['empty_2xx']) - set(base.get('empty_2xx') or []))

    fixed = {}
    for name, fields in b_unbacked.items():
        gone = sorted(set(fields) - set(now['unbacked'].get(name, [])))
        if gone:
            fixed[name] = gone

    total_now = sum(len(v) for v in now['unbacked'].values())
    total_base = sum(len(v) for v in b_unbacked.values())
    print(f"contract gate: {len(now['unbacked'])} capabilities / {total_now} unbacked fields "
          f"(baseline {len(b_unbacked)} / {total_base})")

    if fixed and not quiet:
        n = sum(len(v) for v in fixed.values())
        print(f"\n  {n} field(s) now backed that the baseline still lists — tighten it with "
              f"--write-baseline:")
        for name, fields in sorted(fixed.items()):
            print(f"    {name}: {', '.join(fields)}")

    if not (added or new_no2xx or new_empty):
        print("\nno new disagreements. OK")
        return 0

    print("\nNEW disagreements, not in the baseline:")
    for name, fields in sorted(added.items()):
        print(f"  {name}: {', '.join(fields)}")
    for name in new_no2xx:
        print(f"  {name}: declares no 2xx response at all")
    for name in new_empty:
        print(f"  {name}: 2xx schema is the empty literal {{'type': 'object'}}")
    print("\nEither give the field a home in the response schema, or drop it from `returns`. "
          "If it is genuinely known and unfixable here, record it with --write-baseline and "
...
if __name__ == '__main__':
    ...
    if '--write-baseline' in argv:
        ...
        with open(out, 'w') as fh:
            ...
            fh.write('\n')
        n = sum(len(v) for v in data['unbacked'].values())
        print(f"wrote {out}: {len(data['unbacked'])} capabilities / {n} unbacked fields, "
              f"{len(data['no_2xx'])} without a 2xx, {len(data['empty_2xx'])} empty 2xx")
        sys.exit(0)

    if '--baseline' in argv:
        sys.exit(against_baseline(index, argv[argv.index('--baseline') + 1], quiet))

    sys.exit(main(index, quiet))
