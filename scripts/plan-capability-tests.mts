 ...
 * `get_test_run` they have created a run, and the id is theirs rather than borrowed.
 *
 ...
 * names are inconsistent enough that `list_root_folders` is a listing.
 */
function role(c: Capability): [number, string] {
  ...
  // SCOPE, not as the thing being addressed — counting it made `create_root_folder`
