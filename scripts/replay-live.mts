...
// REQUIRED, no default: a replay re-issues recorded calls, several of which write, so the
// host is not something to guess at. It also kept an internal hostname in a public repo.
const baseUrl = (process.env.CAPABILITY_REGISTRY_BASE_URL_TM || "").trim();
if (!baseUrl) {
  throw new Error(
    "CAPABILITY_REGISTRY_BASE_URL_TM is required: replay re-issues recorded calls, " +
      "including writes, so it will not guess a host.",
  );
}
