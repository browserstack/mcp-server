...
/**
 * Is this capability withheld from the surface?
 *
 * FAILS CLOSED: any truthy `disabled` hides it, not just `=== true`. The flag exists to
 * suppress something that cannot work, so an unexpected shape must hide it rather than be
 * ignored — the opposite default would turn a typo into a silently re-enabled capability.
 */
export function isDisabled(capability: Capability): boolean {
  return Boolean(capability.disabled);
}
...
/**
 * Remove withheld names from `entities[*].capabilities[]`.
 *
 * describeEntity returns the entity doc WHOLESALE (`{ ...doc }`), so a name left here is a
 * name the caller sees, looks up, and cannot invoke — the surface would be advertising the
 * capability in the one place that is not a search result.
 *
 * An entity whose list empties is KEPT, not dropped. It still describes a real thing the
 * product models — its id convention, its relations and its key facts remain true and are
 * what a caller resolving an id actually needs — and removing entities would change what
 * listProducts routes on. No tm entity empties today (attachment keeps 4 of 7, dataset 7
 * of 8); this is the rule for when one does.
 */
function stripDisabledFromEntities(
  bundle: ProductIndex,
  withheld: Map<string, Capability>,
): Record<string, EntityDoc> {
  const entities: Record<string, EntityDoc> = {};
  for (const [entity, doc] of Object.entries(bundle.entities || {})) {
    const listed = (doc as EntityDoc)?.capabilities;
    if (!Array.isArray(listed)) {
      entities[entity] = doc;
      continue;
    }
    const kept = listed.filter(
      (name) => typeof name !== "string" || !withheld.has(name),
    );
    entities[entity] =
      kept.length === listed.length ? doc : { ...doc, capabilities: kept };
  }
  return entities;
}
...
export class CapabilityRegistry {
  ...
  /**
   * product -> name AND "METHOD /path" -> the capability that flag withheld.
   *
   * Kept only so an invoke against a handle the caller already holds can say
   * `capability_disabled` instead of `unknown_capability`. Nothing else reads it, and it
   * is deliberately not reachable from `index`.
   */
  private readonly withheld = new Map<string, Map<string, Capability>>();
  ...
  constructor(
    ...
    // DISABLED CAPABILITIES ARE REMOVED HERE, ONCE, and never reach the rest of the server.
    //
    // The alternative — a `disabled` check at each read site — was rejected: there are four
    // readers of `bundle.capabilities` today (three in search, one in discovery) plus the
    // entity docs, and the cost of a missed one is a capability that is hidden from search
    // and still invokable, which is the exact failure the flag exists to prevent. Filtering
    // the model itself makes the invariant structural instead of remembered.
    ...
    for (const [product, bundle] of Object.entries(index.products)) {
      ...
      const withheld = new Map<string, Capability>();
      const enabled: Capability[] = [];

      for (const capability of bundle.capabilities) {
        // UNIQUENESS IS CHECKED OVER EVERY CAPABILITY, disabled ones included. A clash
        // between a disabled name and a live one is still a broken artifact, and skipping
        // the disabled half would mean re-enabling the flag could fail the load later,
        // somewhere with less context than here.
        if (isDisabled(capability)) {
          withheld.set(
            endpointKey(capability.method, capability.path),
            capability,
          );
          if (capability.name) withheld.set(capability.name, capability);
        } else {
        ...
        if (!isDisabled(capability)) names.set(capability.name, capability);
      }

      this.byEndpoint.set(product, lookup);
      this.byName.set(product, names);
      this.withheld.set(product, withheld);
      products[product] = withheld.size
        ? {
            ...bundle,
            capabilities: enabled,
            entities: stripDisabledFromEntities(bundle, withheld),
          }
        : bundle;
    }
  ...
  /**
   * The `capability_disabled` message for a handle this build withholds, if it is one.
   *
   * Says THAT it is disabled and nothing about why. The reason is documentation and lives
   * with the decision; repeating it here would put four copies of a paragraph in the
   * shipped artifact to serve a path the flag itself makes rare.
   *
   * Returns undefined for anything else, so the caller falls through to its own
   * not-found wording.
   */
  private withheldMatch(handle: string, product?: string): string | undefined {
    for (const [owner, withheld] of this.withheld) {
      if (product && owner !== product) continue;
      if (!withheld.has(handle)) continue;
      return (
        `capability_disabled: ${handle} is disabled on this surface. This is ` +
        `deliberate — it is withheld by the ${owner} index, not missing from it — so ` +
  ...
  /** How many capabilities each product withholds. Startup logging only. */
  disabledCounts(): Record<string, number> {
    const out: Record<string, number> = {};
    for (const [product, withheld] of this.withheld) {
      // Each capability is keyed twice when it has a name — by endpoint and by name.
      const distinct = new Set(withheld.values());
      if (distinct.size) out[product] = distinct.size;
    }
    return out;
  }
  ...
  byNameLookup(
    ...
    if (matches.length === 0) {
      // WITHHELD IS A DIFFERENT ANSWER FROM ABSENT, and only here. Search cannot surface a
      // disabled capability, so reaching this branch means the caller already held the
      // exact handle — from an older index, a doc, or a human. "unknown_capability" would
      // send it hunting for a synonym that does not exist; the reason ends the attempt.
      const off = this.withheldMatch(name, product);
      if (off) throw new InvocationError(off);
  ...
  byEndpointLookup(
    ...
    if (matches.length === 0) {
      const off = this.withheldMatch(key, product);
      if (off) throw new InvocationError(off);
      throw new InvocationError(
