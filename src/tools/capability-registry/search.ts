 ...
 * consistent — the endpoint that lists folders is `list_root_folders` — so the path's
 ...
 * again rewards verbose names for repeating themselves: `list_test_run_test_cases_by_integer_id`
 ...
 * route is mostly its verb, which tm applies inconsistently (`list_root_folders` lists,
 * `list_folder_test_cases` also lists), so the verb is noise as often as signal.
 *
