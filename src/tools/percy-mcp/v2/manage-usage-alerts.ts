...
export async function percyManageUsageAlerts(
  args: { org_id: string; action?: string; threshold?: number; emails?: string; enabled?: boolean; product?: string },
  ...
  return { content: [{ type: "text", text: `Usage alerts ${action}d. Threshold: ${args.threshold || "default"}` }] };
}
