...
export async function percySearchBuildItems(
  args: { build_id: string; category?: string; browser_ids?: string; widths?: string; os?: string; device_name?: string; sort_by?: string; limit?: number },
  ...
  // Array filters
  if (args.browser_ids) args.browser_ids.split(",").forEach(id => { params[`filter[browser_ids][]`] = id.trim(); });
  if (args.widths) args.widths.split(",").forEach(w => { params[`filter[widths][]`] = w.trim(); });
  ...
  if (!items.length) {
    return { content: [{ type: "text", text: "No items match the specified filters." }] };
  }
  ...
  items.forEach((item: any, i: number) => {
    ...
    const diff = attrs.maxDiffRatio != null ? `${(attrs.maxDiffRatio * 100).toFixed(1)}%` : "—";
