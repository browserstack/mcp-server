...
| --- | --- | --- | --- |
| folder `__mcp-probe-createrootfolder-20260921` | 765811 | `create_root_folder` | yes — `delete_folder_and_contents` (still to be probed) |
| configuration `__mcp-probe-config-20260921` | 5189868 | `create_configuration` | **NO API PATH — UI only** |

## The configuration one is not cleanable, and that is the finding

`create_configuration` is reached through a project but writes **group-scoped**. Confirmed
two ways: the new row carries `group_id: 3452`, the same as every existing configuration, and
a follow-up `list_configurations` against PR-2005 shows the pool going **244 → 245**.
...
| object | id | created by | removable? |
| --- | --- | --- | --- |
| test run `__mcp-probe-createrun-20260921` | TR-9110 / uuid 17577900 | `create_test_run_by_integer_id` | yes — `delete_test_run` (still to be probed) |
| test case `__mcp-probe-createcase-20260921` | TC-54484 / int 1985018 | `create_test_case_by_integer_id` | yes — `bulk_delete_test_cases` (still to be probed) |
...
`create_test_plan_by_integer_id` created nothing — it 500s. Verified: the project still shows 29 active plans.
...
| custom field `__mcp-probe-cfdef-20260921` | 106131 (+ dataset 31319) | `create_custom_field_definition` | **delete refused by the invoke runtime** |
...
| shared step `__mcp-probe-sharedstep-20260921` | 27846 | `create_shared_step` | yes — `delete_shared_step` (still to be probed) |
...
| exploratory session `__mcp-probe-exploratory-clone-20260921` | **780** (ES-15) | `clone_exploratory_session`, cloned from source **779** | yes — `delete_exploratory_session` (still to be probed) |
...
| dataset `__mcp-probe-cfdataset-20260921` | 31320 (options 146668/146669), hung off custom field 106131 | `create_custom_field_dataset` | **delete refused by the invoke runtime** — `delete_custom_field_dataset` exists and is well-specified, but its mode is `destructive`, so invokeCapability refused it outright (verbatim: "delete_custom_field_dataset is a destructive operation and is not available through this surface"). Same non-cleanable pattern as field 106131 itself. Dataset 31320 has no projects linked (created deliberately unlinked to avoid the "project already on another dataset of this field" conflict with dataset 31319, which already owns PR-2005/379335744) — it is invisible in every project but exists and is reusable as the dataset id for probing `update_custom_field_dataset_project_mapping`.
...
| exploratory clone `__mcp-probe-exploratory-clone-20260921` | 780 (ES-15) | `clone_exploratory_session` | yes — `delete_exploratory_session` |
| report `__mcp-probe-report-20260921` | 10393 / SC-492 | `create_report_by_integer_id` | **no delete-report capability in this profile** |
| project `__mcp-probe-project-20260921` | **379339429 / PR-2011** | `create_project` | **NO API PATH — UI only** |

`create_project` is the second and worse instance of the no-cleanup pattern. A
...
orphaned project.
| subfolder `__mcp-probe-subfolder-20260921` | 765817 (parent 765811) | `create_sub_folder` | delete refused at runtime |
| test plan `__mcp-probe-planclone-20260921` | 48319 / TP-1632 | `clone_test_plan` | delete refused at runtime |
| test run TR-9111 | created as a side effect of the plan clone | `clone_test_plan` | delete refused at runtime |
| custom-field dataset (2nd) | 31320, options 146668/146669 | `create_custom_field_dataset` | delete refused at runtime |
| project settings on PR-2011 | `tc_sharing`, `preferred_jira_host` | `update_project_settings` | PR-2011 is throwaway |
| custom-field dataset option `__mcp-probe-cfoption-20260921` | 146670, dataset 31320, custom field 106131 | `create_custom_field_dataset_option` | yes — `delete_custom_field_dataset_option` exists (mode `destructive`, would be refused by the invoke runtime like every other destructive call so far) |
...
| tags `__mcp-probe-mergetag-20260922-a` / `-b` | 541125, 541126 | `merge_tags` (unmerged) | both live on TC-54486 |
| test case TC-54502 (copy of TC-54489) | 1985215, in folder 765817 | `bulk_copy_test_cases` | delete refused at runtime |
| test plan TP-1632 left renamed `__mcp-probe-planclone-20260921-updated` | 48319 | `update_test_plan_by_integer_id` | disposable probe plan |

## `update_custom_field_dataset_project_mapping` probe (2026-09-22) — dataset mappings now stable
...
live by `get_custom_field_dataset` reads on both dataset ids after the corrective calls, not
inferred from write responses. See `tests/live/runs/tm/update_custom_field_dataset_project_mapping.json`
for full detail.
...
## `update_custom_field_dataset_option` probe (2026-09-22) — dataset 31320's default moved again
...
default"). See `tests/live/runs/tm/update_custom_field_dataset_option.json` for the full
...
silent-discard 200 (`create_bulk_test_cases`). Verified nothing was created.
...
| object | id | created by | removable? |
| --- | --- | --- | --- |
| test result (status Blocked) on TC-54486 in TR-9110, mapping_id 2150424053 | 2150424064 | `create_test_result_for_test_case` (setup, since TC-54486 had never been executed in this run) | yes — `delete_test_result` (not probed here) |
...
## `bulk_restore_archived_test_cases` probe (2026-09-22) — net no-op, but flags a stale fixture premise
...
folder 764138 — neither `list_archived_test_cases` nor `list_binned_test_cases` listed them. If
...
confirmed the archive with a read, then restored them with `bulk_restore_archived_test_cases`. End state is
...
documented for this route. See `tests/live/runs/tm/bulk_restore_archived_test_cases.json` — verdict `DRIFT`
...
## `clone_test_run_by_integer_id` probe (2026-09-22) — TR-9110 cloned to TR-9120; results confirmed NOT copied

| object | id | created by | removable? |
| --- | --- | --- | --- |
| test run `__mcp-probe-runclone-20260922` | TR-9120 / uuid 17578918 | `clone_test_run_by_integer_id`, cloned from source TR-9110 | yes — `delete_test_run` (still to be probed) |
...
independent reads (`list_test_runs_by_integer_id`, `list_test_run_test_cases_by_integer_id`), not the clone's own inline
response, which under-reported its own result as `test_cases_count:0` / all-zero progress moments
after the 5 cases were already live (same defect family as `clone_exploratory_session`). TC-54485's
...
`clone_test_run_by_integer_id`'s own write response gets `project_id`/`is_dynamic`/`overall_progress` right (string /
boolean / real object), matching `create_test_run_by_integer_id`; a `list_test_runs_by_integer_id` re-read of the same new run
gets all three wrong again (integer, integer, **null** — not even a zeroed object). See
`tests/live/runs/tm/clone_test_run_by_integer_id.json` — verdict `DRIFT` (stale inline response; `created_by` /
...
TR-9110 concurrently in this shared fixture, not to `clone_test_run_by_integer_id`. Anyone probing TR-9110 next
...
The `clone_test_run_by_integer_id` probe flagged that TR-9110 had drifted from this ledger's recorded state.
It is explained, not mysterious:

- TC-54485 now appears **twice**, at mapping ids **2150424076 / 2150424077**, and the previously
  recorded 2150424062 is gone. That is `create_step_result`, which persisted **two** case-level
...
## `close_test_run` probe (2026-09-22) — TR-9111 closed, first closed run in this project

Closed TR-9111 (uuid 17578623) as instructed — the orphaned run left over from the earlier
`clone_test_plan` side effect, unlinked from its plan, holding 1 case. TR-9110, TR-9120,
...
Before this probe, project-wide `list_test_runs_by_integer_id` `count` had read `{active: 33, closed: 0}` on
every check across the whole campaign. After closing: `{active: 32, closed: 1}` — verified against
actual rows (TR-9111 dropped off the active listing), not just the count object. `list_closed_test_runs`,
`get_closed_test_runs_info` and `get_closed_test_runs_split` all now return real, non-empty
data for the first time — see `tests/live/findings/close_test_run.md` for the shapes.
...
**Self-misreporting on the close's own response**: `close_test_run`'s 200 returned
...
family of defect as `clone_test_run_by_integer_id`'s stale `test_cases_count`. See
`tests/live/runs/tm/close_test_run.json` and `tests/live/findings/close_test_run.md` —
verdict `DRIFT`.
...
Also on preprod, from the create_template shape probing: several `__mcp-probe-tmpl-*` attempts,
...
## Preprod residue — 2026-09-22 (send_report_email_now probe)
...
Workspace-level, created while verifying `create_template`:
...
this surface — `delete_template` is `mode: destructive` and refused before binding.
