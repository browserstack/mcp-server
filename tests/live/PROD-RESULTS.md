...
| --- | --- | --- | --- |
| `list_tags_v3` | BLOCKED | **BLOCKED** | **yes — escalation vindicated** |
| `get_test_run_v2` | BLOCKED *(stale)* | **PASS** | 5/5 2xx; drift kept in notes |
| `test_case_results_v1` | BLOCKED | **BLOCKED** | **yes — and now stronger** |
| `verify_test_case_tags_v1` | BLOCKED | **BLOCKED** | **yes — byte-identical** |
| `get_test_run_progress_v1` | UNVERIFIED | **PASS** | 3/3 clean 2xx |
| `list_binned_test_cases_v1` | DRIFT | **PASS** | 2xx; the `returns` gap is tracked as an index fix |
| `get_test_results_for_test_case_v2` | DRIFT | **PASS** | no — the index corrections were right |
| `get_shared_components_v1` | UNVERIFIED | **PASS** | resolved — prod had the data |
| `linked_test_cases_selection_v1` | UNVERIFIED | **PASS** | resolved — prod had the data |
| `folder_attachments_v1` | UNVERIFIED | **PASS** | 294/294 folders returned 2xx |
| `validate_bdd_export_selection_v1` | UNVERIFIED | **UNVERIFIED** | still empty after ~3,125 cases |
...
| --- | --- |
| `get_test_run_v2` | `issues` declared "key absent when no association", returns `[]` present-empty. `minify` is a no-op on preprod, functional on prod |
| `list_binned_test_cases_v1` | `template`, `created_by`, `creator_details` returned but absent from the flat `returns` list — **the proof case for the bidirectional CI gate** |
...
Moved UNVERIFIED -> PASS: `folder_attachments_v1` (294/294 folders, all
`200 {success:true, attachments:[]}`), `get_test_run_progress_v1` (3/3 clean 200s).

**Not moved, and why:**
- `list_binned_test_cases_v1` returns 2xx but carries a **confirmed static contract defect** —
...
- `validate_bdd_export_selection_v1` **never returned a 2xx at all** — only the 400 failure
...
envelope is correct, not that the item shape is right. For `folder_attachments_v1` the ten
...
for `merge_tags_v1`, and the tag surface has a **separate** capability, `list_test_case_tags_v1`
— which has its own confirmed 500 on the `q` filter. The claims may belong to that capability,
not this one. Either way they are not properties of `list_tags_v3`, and the preprod note has
been corrected.


## `get_test_run_v2` — the preprod premise was wrong, and it was our own record's fault
...
been retracted on 2026-09-18** in `findings/get_test_run_v2.md`: a direct re-test returned 200,
...
**`test_case_results_v1`** — 3 attempts across two real cases, varying `issue_type` and paging.
...
**`verify_test_case_tags_v1`** — 3 attempts: an empty `tags:[]` (the contract's own documented
valid case), a mixed known+new tag, and a single new tag. All three returned the **byte-identical**
bare 500. A control call to `list_test_case_tags_v1` on the same project returned **200 with 31
...
`list_tags_v3`'s crash. That is only half right. They share a *class* — an uncaught handler
exception surfacing as a raw 500 conforming to neither declared schema — but the literal bodies
differ: these two return `{"status":500,"error":"Internal Server Error"}`, while `list_tags_v3`
...
## `get_test_run_progress_v1` — the flake judgement was right
...
## `list_binned_test_cases_v1` — the half-fix traced to its commit
...
ignored on `get_root_folders_v1` on this same project — so that bug is per-capability, not
project-wide.


## `get_test_results_for_test_case_v2` — PASS, and it validates the index corrections
...
**Incidental:** `search_test_results_v1` reproduced its documented "failures swallowed,
...
**`get_shared_components_v1` → PASS.** The very first unfiltered call returned **21 populated
...
**`linked_test_cases_selection_v1` → PASS**, and it got there by ignoring my hints. Both runs I
...
**`folder_attachments_v1` → UNVERIFIED after 12 folders** across 4 branches — both folders I
...
silent-ignore failure mode seen on `get_root_folders_v1` does not apply here — strict rejection
is the better behaviour and worth noting as the contrast.

**`validate_bdd_export_selection_v1` → UNVERIFIED after searching ~3,125 of 16,267 cases (~19%)**
...
## `list_binned_test_cases_v1` — RESOLVED 2026-09-22, the user binned cases
...
| `limit` honoured | asked 5, got `page_size: 5` (unlike `per_page` on `get_root_folders_v1`) |
...
| capability | preprod | prod | reproduces? |
| --- | --- | --- | --- |
| `get_projects_basic_v1` | DRIFT | **PASS** | **no — preprod-only regression** |
| `get_projects_minify_v1` | DRIFT | DRIFT | yes |
| `list_projects_v1` | DRIFT | DRIFT | yes |
| `search_project_entities` | DRIFT | DRIFT | yes |
| `get_entity_filter_details_v1` | DRIFT | DRIFT | yes — both open issues confirmed |
| `get_issues_count_info_v1` | DRIFT | DRIFT | yes — **positional array, now with real data** |
| `get_closed_test_runs_info_v1` | DRIFT | DRIFT | yes — **positional array, now with real data** |
| `get_test_case_count_trend_v1` | DRIFT | DRIFT | yes, but a **different** drift than briefed |
| `get_test_case_type_split_v1` | DRIFT | DRIFT | yes, but a **different** drift than briefed |
...
- **Genuinely positional** — `get_issues_count_info_v1` and `get_closed_test_runs_info_v1`.
...
- **NOT positional** — `get_test_case_count_trend_v1` returns a genuine object map keyed by
  case-type name, and `get_test_case_type_split_v1` returns a clean `{name,y,value,field}`
  object array. Both match their schemas on shape.

Their real drifts are different and were only visible at prod volume:

- `get_test_case_count_trend_v1`: `data.Total` is **missing the required `field` key**, and
  `Total.value` is `[null]` — wrong-typed. Found across ~45 real case-type keys.
- `get_test_case_type_split_v1`: the guidance claims **"`y` and `value` are the same count
...
`get_projects_basic_v1.q[query]`, `list_projects_v1.q` and `search_project_entities.q[query]`
all genuinely narrow. `get_projects_minify_v1` has no filter param at all. The defect is
**isolated to the v2 listing**.

### `get_entity_filter_details_v1` — both open issues confirmed, and precisely distinguished
...
| --- | --- | --- | --- |
| `list_archived_test_plans_v1` | BLOCKED | **PASS** | **account limitation, not a defect** |
| `get_archived_test_plan_count_v1` | BLOCKED | **PASS** | **account limitation, not a defect** |
| `list_test_plans_v1` | DRIFT | DRIFT | reproduces |
| `get_test_plan_by_integer_id_v1` | DRIFT | DRIFT | reproduces |
| `get_test_plan_test_runs_v1` | DRIFT | DRIFT | reproduces |
| `get_test_plan_execution_trend_v1` | DRIFT | DRIFT | reproduces, now with real volume |
...
The same flag gates `bulk_archive_test_plans_v1` and `bulk_retrieve_test_plans_v1`, which were
...
**I was wrong about this, repeatedly and confidently.** I described `get_test_runs_v1` returning
...
`get_test_plan_test_runs_v1`, then located that same run two pages into
`get_test_runs_v1(include_test_plan=true)` and found it correctly carrying
...
A narrower gap **does** reproduce on both: `get_test_plan_test_runs_v1`'s own row-level
`test_plans` field is always empty, even on rows returned precisely because they are linked.

### NEW — `minify=true` zeroes real data on `list_test_plans_v1`
...
Note the contrast with `get_test_run_v2`, where `minify` is a no-op on preprod and merely drops
keys on prod. Here it corrupts a value.

### Positional array — reproduces with far better evidence

`get_test_plan_execution_trend_v1` declares `test_plans_execution_trend_graph` as a bare object;
...
### `search_test_cases_by_folder_v1` — genuinely dead, now beyond argument
...
present on five capabilities and absent on `get_system_field_values_v1` — per-capability, not
entity-wide, exactly as preprod found.

### Two new findings only visible at prod scale

- **`search_project_entities_v2`'s `folder_id` filter is non-recursive.** It returns 972 for a
  folder whose true recursive total is 1,168. Invisible at preprod's 2-row scale.
- **`get_system_field_values_v1` has unreliable paging metadata.** Non-final pages under-fill
...
The sharpest result here. `create_report` and `update_report` were found on preprod to share one
...
And the diagnosis is now precise: **`list_reports_v2` DOES return `mail_to` on every row.** So
...
`list_schedules` resolved **SC-12541 → integer id 95008**. `list_reports_v2` exposes only the
...
`get_custom_field_dataset_v2` and `list_workspace_fields_v2` both return
...
(`get_custom_field_v2`, `list_custom_fields_v2` too), while the **v1 project-form endpoints work
...
and the undeclared `options.info` envelope), and whether `list_workspace_fields_v2`'s `info`
shape still varies by mode.

### A third descriptor variant

`get_custom_field_values_v1` returns
...
sub-variant noted on `get_project_id_custom_fields_v2_v1`.

### Confirmations worth having

**`get_project_id_custom_fields_v2_v1`** still returns `system_fields` as an **object** against a
...
**The `get_system_field_values_v1` paging bug does not spread** — `get_custom_field_values_v1`
...
### `get_test_case_history_diff_v1` — a **fifth** category: a genuinely broken route
...
`get_test_case_histories_v1`, all with real diffs. The diff call still **404s on prod**,
...
I suggested the `list_test_case_tags_v1` `q`-filter 500 might have been **misattributed**, after
a related claim about `list_tags_v3`'s `search` turned out false on prod. **That speculation was
wrong**, and the probe checked rather than accepting it: `findings/5xx-declared-inputs.md` and
`findings/update_tag_v1.md` both name this capability explicitly, by name *and* by its literal
route (`.../test-case/tags-v2?q=`). It was found while cross-checking against `list_tags_v3` but
...
preprod's constant `"api"`; and `get_test_case_history_v1` drops `version_name` for a
...
`get_root_folders_v1` returns a plain integer matching its declared type. `get_folder_tree_v1`
declares `group_id` and **never emits it** on any of 447 nodes.

### `per_page` is a footgun, not an outright ignore

More precise than my earlier note: on `get_root_folders_v1`, `per_page` **alone** is silently
...
`get_folder_tree_v1` has no paging at all (447 nodes in one shot). `list_folder_contents`
accepts only `p`; `per_page`/`limit` are rejected client-side.

### `get_test_case_attachments_v1` — BLOCKED, and proven data-independent

The probe did the hard version of this. Rather than concluding from an empty result, it found
that `search_project_entities_v2` embeds an **undocumented `attachments[]`** on each case row,
...
`get_folder_tree_v1` returns **`total_cases_count: 0` on all 447 nodes** — including flat leaf
...
That joins `search_project_entities_v2`'s non-recursive `folder_id` filter (972 vs a true 1,168)
— **two independent ways the recursive-count concept is unreliable**, while
`get_root_folders_v1` and `list_folder_contents` both report it correctly (972 / 1168 / 36 exact).

## Test run reads — 4 DRIFT, all reproducing, three new defects

### `get_test_run_cases_v1` — the guidance was fixed, the contract was not
...
as the `list_binned_test_cases_v1` one.
...
### `get_test_cases_for_v1_test_run` — grain confirmed, correctness gap found
...
### `get_test_runs_form_fields_v1` — a third descriptor shape, and silent data loss
...
The part worth routing: **`get_closed_test_runs` still serves cached 1-Blocked progress
