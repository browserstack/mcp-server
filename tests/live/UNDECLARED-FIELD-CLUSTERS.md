...
| no row schema at all | `search_project_entities_v2` |
...
- **`get_test_runs_selection`** returns the **raw case record**: `automation_status` is a plain
...
- **`get_test_cases_for_v1_test_run`** and **`bulk_assign_test_cases_to_test_run`** return a
...
1. **`TestCase`** — the 13 cluster-1 fields declared once, each carrying the routes it is
   ...
   `get_test_cases_v1`, `list_folder_test_cases_v1` and the mapping rows. Stating that in the
...
4. **`search_project_entities_v2`** — given the row schema it never had.
...
6. **`TestRunSelectionCaseRow`** — new, 32 fields, and `get_test_runs_selection`'s `returns`
...
`get_test_case_detail` 52/52 · `get_test_case_by_integer_id_v1` 47/47 · `get_test_cases_v1` 46/46 ·
`list_folder_test_cases_v1` 47/47 · `search_project_entities` 47/47 ·
`search_project_entities_v2` 47/47 · `get_test_runs_selection` 32/32 · `get_test_runs_v1` 32/32 ·
`get_test_run_by_integer_id_v1` 32/32 · `get_exploratory_session` 28/28 ·
`list_exploratory_sessions` 16/16 · `get_root_folders_v1` 9/9

One capability is recorded as **BLOCKED**. `get_test_cases_for_v1_test_run` cannot pass in
...
| --- | --- |
| `get_test_case_by_integer_id_v1` | 46 |
| `get_test_case_detail` | 29 |
| `list_folder_test_cases_v1` | 29 |
| `search_project_entities` | 24 |
| `get_test_cases_v1` | 23 |
| `create_root_folder_v1` | 22 |
| `get_test_runs_selection` | 20 |
| `search_project_entities_v2` | 19 |
| `get_test_cases_for_v1_test_run` | 15 |
| `list_projects_v1` | 15 |
| `get_test_plan_test_runs_v1` | 14 |
...
`search_project_entities_v2`, where a probe used that undocumented field to establish that
**156 of 203 sampled cases carry real attachments** (PDFs and PNGs with filenames, sizes and
checksums).

Meanwhile `get_test_case_attachments_v1` — the capability whose entire job is attachments —
