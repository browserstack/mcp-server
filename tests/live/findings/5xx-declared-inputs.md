...
- **Product:** tm
- **Environment:** originally preprod only; `list_tags` is now confirmed in **production** too (see escalation)
...
> **One entry was removed on 2026-09-22.** `get_test_run` / `minify=true` was listed here as a
> third crash. It should not have been: the underlying finding was **retracted on 2026-09-18**
> (see `findings/get_test_run.md`, Finding 7) as a preprod flakiness burst, and this file was
...
for `list_test_case_tags`, but `list_tags` has since lost it by failing on every value.

| capability | the declared input | result | control that works |
| --- | --- | --- | --- |
| `list_tags` | **every** `entity_type` value | 500 | **no control left — see escalation below** |
| `list_test_case_tags` | `q=<term>` | 500 Internal Server Error | same call paged without `q` → 200 |

## ESCALATION 2026-09-22 — `list_tags` now fails on every declared input
...
## 1. `list_tags` — originally only `entity_type=test_recording`
...
## 2. `list_test_case_tags` — the `q` filter
...
**Provenance, stated plainly:** this was found incidentally in `update_tag`'s setup, while
...
- **`get_test_run_progress`** — null-body 500s on attempts 2-3. An unrelated capability
  (`list_test_runs_by_integer_id`) hit the identical null-body 500 in the same window and recovered on its
...
- **`assign_test_run_test_cases`** — three 500s, all in `setup[0-2]`. Two of those were
