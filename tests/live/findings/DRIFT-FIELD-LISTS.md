...
`create_folder` is the clearest trap: a naive set difference makes it look like eight
...
kept for provenance. Nothing is asked of you for these:
  ...
  edit_project            dropped access_type, user_role, permissions
  get_custom_field        added placeholder, system_name, is_bulk_editable,
                             is_filterable, group_id
  list_custom_fields      added placeholder
  list_configuration_groups  dropped record_status, updated_at
  unlink_test_run         guidance only — how to tell you need build_id

That leaves 29 capabilities across 19 schemas genuinely needing adjudication.

## (inline body)  (6 capabilities)

### assign_test_run_test_cases  —  POST /api/v2/projects/{project_id}/test-runs/{test_run_id}/assign
- **declared** (`returns`): success
- **observed** (top-level keys): success

### bulk_update_test_run_test_cases  —  PATCH /api/v2/projects/{project_id}/test-runs/{test_run_id}/test-cases
- **declared** (`returns`): success, async, unique_id
- **observed** (top-level keys): async, success, unique_id

### edit_project  [ALREADY FIXED — no action]  —  POST /api/v1/projects/{project_id}/edit
...
- **declared, absent from response**: access_type — listed in edit_project's `returns` and in its own response schema properties, but absent from the actual 200 body on both the edit call and the plain get_project_by_integer_id read of the same project, user_role — listed in `returns` for edit_project but absent from the edit response (though present when reading the project via get_project_by_integer_id), permissions — same: listed in `returns` for edit_project but absent from the edit response (present on a plain read)

### unlink_test_run  [ALREADY FIXED — no action]  —  POST /api/v1/integrations/{issue_type}/unlink-test-run
- **declared** (`returns`): success, test_runs, info
- **observed** (top-level keys): info, success, test_runs

### update_tag  —  PATCH /api/v1/projects/{project_id}/tags/{id}
...
### update_test_case  —  PATCH /api/v2/projects/{project_id}/test-cases/{test_case_id}
...
### create_test_plan  —  POST /api/v2/projects/{project_id}/test-plans
...
### get_test_plan  —  GET /api/v2/projects/{project_id}/test-plans/{test_plan_id}
...
### create_folder  —  POST /api/v2/projects/{project_id}/folders
...
### rename_folder  —  PATCH /api/v2/projects/{project_id}/folders/{folder_id}
- **declared** (`returns`): id, name, description, parent_id, cases_count, sub_folders_count, links, urls
- **observed** (top-level keys): folder, success

## TestRunWriteV2Response  (2 capabilities)

### replace_test_run  —  POST /api/v2/projects/{project_id}/test-runs/{test_run_id}/update
...
### update_test_run  —  PATCH /api/v2/projects/{project_id}/test-runs/{test_run_id}/update
- **declared** (`returns`): success, testrun, id, uuid, identifier, name, description, owner, assignee, run_state, active_state, is_automation, is_dynamic, tags, configurations, test_plans, test_cases_count, overall_progress, overall_progress_by_status_id, project_id, created_at, updated_at, links
- **observed** (top-level keys): success, testrun
...
### list_test_plan_test_runs  —  GET /api/v2/projects/{project_id}/test-plans/{test_plan_id}/test-runs
...
### list_test_runs  —  GET /api/v2/projects/{project_id}/test-runs
...
### create_test_case_comment  —  POST /api/v1/projects/{project_id}/folder/{folder_id}/test-cases/{test_case_id}/comments
...
### list_test_case_comments  —  GET /api/v1/projects/{project_id}/folder/{folder_id}/test-cases/{test_case_id}/comments
...
### create_test_run_test_case_comment  —  POST /api/v1/projects/{project_id}/test-runs/{test_run_id}/test-cases/{test_case_id}/comments
...
### list_test_run_test_case_comments  —  GET /api/v1/projects/{project_id}/test-runs/{test_run_id}/test-cases/{test_case_id}/comments
...
### get_custom_field  [ALREADY FIXED — no action]  —  GET /api/v2/custom-fields/{id}
...
### list_custom_fields  [ALREADY FIXED — no action]  —  GET /api/v2/custom-fields
...
### list_exploratory_session_form_fields  —  GET /api/v1/projects/{project_id}/exploratory-sessions/form-fields
...
### search_all_projects  —  GET /api/v1/global/search
...
### create_sub_test_plan  —  POST /api/v2/projects/{project_id}/test-plans/{test_plan_id}/sub-test-plans
...
### get_template  —  GET /api/v2/templates/{id}
...
### search_test_results  —  GET /api/v1/test-results
...
### list_test_run_test_cases  —  GET /api/v2/projects/{project_id}/test-runs/{test_run_id}/test-cases
...
### bulk_set_test_result_status  —  PATCH /api/v2/projects/{project_id}/test-runs/{test_run_id}/results
- **declared** (`returns`): success
- **observed** (top-level keys): success

## V2HistoryListResponse  (1 capability)

### get_test_case_histories  —  GET /api/v2/projects/{project_id}/test-cases/{test_case_id}/history
...
### create_report  —  POST /api/v2/projects/{project_id}/reports
...
### create_test_case  —  POST /api/v2/projects/{project_id}/folders/{folder_id}/test-cases
...
### list_test_results_for_test_case  —  GET /api/v2/projects/{project_id}/test-runs/{test_run_id}/test-cases/{test_case_id}/results
...
### create_test_results_for_test_run  —  POST /api/v2/projects/{project_id}/test-runs/{test_run_id}/results
...
After the v1.15 correction, `get_test_run` still has one mismatch: the response carries
