# add_test_case_comment_v1 — DRIFT (copy-pasted entity semantics)
...
- **Run file:** `tests/live/runs/tm/add_test_case_comment_v1.json`
- **Related:** `findings/list_test_run_test_case_comments_v1.md`
...
That description plainly describes the **run-scoped** comment capability (`create_test_run_test_case_comment_v1` / `list_test_run_test_case_comments_v1`), not this one. So the declared value and its prose appear to have been copied across from the run-scoped sibling, where they are presumably correct, onto a capability where they are wrong.
...
`group_id`, `project_id` and `parent_id` appear in the nested schema **and** in the real response, but are **missing from the flat `returns` arrays** of both this capability and `get_test_case_comments_v1`. Consistent with the suite-wide finding that flat `returns` arrays are unreliable in both directions; recorded here as another data point rather than as a separate defect.

## Verified in storage

Baseline confirmed 0 comments on the target triple (project int `379335744`, folder `764828`, case int `1975349`) before the write — matching a sibling's sweep of 18 cases across 3 projects. After the write, `get_test_case_comments_v1` returned **1** comment whose `id`, `created_at` and text matched the write response exactly.

This is a **genuine write**, not a false 2xx.

## Index actions proposed

1. **Correct `entity_type`** to `"TestCase"` on this capability and on `get_test_case_comments_v1`, and rewrite the description — the current text describes the run-scoped capability.
