# assign_test_run_test_cases — DRIFT (the capability cannot perform its function)
...
- **Run file:** `tests/live/runs/tm/assign_test_run_test_cases.json`
- **Related:** `findings/create_test_run.md`, `findings/list_test_run_test_cases` (undeclared `execution_id`)
...
Re-reading `list_test_run_test_cases` **immediately** and again **~15 s later**: both targeted rows still `assignee: null`. Nothing changed. The two untargeted rows were also correctly untouched, so this is not a blanket-update problem — it is a **no-op**.
...
- The server's 400 confirms the field is addressed at `#/assign_to/0/execution_id` — more evidence that `invokeCapability` re-nests flat fields under the declared `json_path`, consistent with `create_test_run`'s findings.
- The only string-shaped `execution_id` a row actually has is the **hex field returned by `list_test_run_test_cases`, which is itself undeclared there** (one of eight undeclared fields on that capability). So the working value is not merely blocked — it is undocumented.
...
`latest_result_id` — which `list_test_run_test_cases` declares, returns, and whose guidance says "use it when you need to fetch or amend that case's recorded result" — is **not accepted by this capability in any form tested.**
...
2. **Declare the row-level `execution_id` hex field on `list_test_run_test_cases`**, since it is the value this capability needs and is currently undocumented.
...
No backend source or logs were consulted, so **why** `test_case_id` is accepted-and-ignored is unknown — it could be an unimplemented branch, a silently-dropped strong parameter (as seen on `update_test_case`'s `is_shared`), or a permissions filter. What is established is the request/response evidence above plus two independent storage re-reads confirming no change.
