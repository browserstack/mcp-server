...
| --- | --- | --- | --- |
| `create_bulk_test_cases_v1` | **silent half-success** — bad `folder_id` discarded, row filed into the path folder instead, no signal | 200 | no |
...
| `bulk_edit_test_cases` | **whole call fails**, retry with the valid id alone succeeds | 404, bare `{success:false}` | no — 404 absent from its declared surface entirely |
| `bulk_assign_test_cases_to_test_run` | **whole call fails** | 502, `{success:false, message:"Failed to update assignee"}` | no — and its guidance promises a 200 no-op |
...
and `create_bulk_test_cases_v1` returns **200** and files the row somewhere else.
...
`bulk_copy_test_cases` and `create_bulk_test_cases_v1` report `success: true` while doing part
...
This matters more because the product **does** have the concept — `get_test_cases_for_v1_test_run`
...
- `create_bulk_test_cases_v1` → **200**, bad id silently discarded and the case **filed into a
...
`bulk_edit_test_cases.json`, `create_bulk_test_cases_v1.json`,
`bulk_assign_test_cases_to_test_run.json`, `copy_folder.json`.
