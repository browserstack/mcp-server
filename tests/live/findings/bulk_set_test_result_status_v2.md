# bulk_set_test_result_status_v2 — DRIFT (moderate severity, undocumented behavior)
...
- **Run file:** `tests/live/runs/tm/bulk_set_test_result_status_v2.json`
- **Related:** `findings/bulk_update_test_run_test_cases_v2.md` — same duplicate mapping (`TC-54457` / mapping ids `2150387252`, `2150387283`), same `TR-9062`, same missing-`mapping_id` gap
...
2. **Consider a `mapping_id`/`execution_id` alternative targeting param**, mirroring the v1 result-create capability, so a caller can disambiguate duplicate mappings the way `bulk_update_test_run_test_cases_v2`'s own duplicate-creation bug now makes necessary in this exact run.
