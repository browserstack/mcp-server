# bulk_update_test_run_test_cases_v2 — DRIFT (high severity, functional)
...
- **Run file:** `tests/live/runs/tm/bulk_update_test_run_test_cases_v2.json`
- **Related:** `findings/assign_test_run_test_cases_v2.md` — the same silent-success failure mode

## What this capability actually is

Worth stating up front, because the probe brief got it wrong: its body has exactly three fields — `add_test_cases`, `remove_test_cases`, `preserve_existing_results`. **It is a case-membership endpoint, not a status setter.** There is no status field and no `execution_id` parameter, so the `execution_id` integer/string catch-22 that makes its sibling `assign_test_run_test_cases_v2` unusable does **not** apply here. Rows are addressed by `test_case_ids` (`TC-NNN` strings) plus optional integer `configuration_ids`.
...
| --- | --- | --- |
| `assign_test_run_test_cases_v2` | `200 {success:true}` | none — rows unchanged (2 re-reads) |
| `bulk_update_test_run_test_cases_v2` (remove) | `202 {success:true, async:true, unique_id}` | none — rows unchanged (3 re-reads, 45 s) |
...
- One status-0 preprod burst on `get_test_run_test_cases_v2` was confirmed as environment noise via a successful `list_test_runs_v2` control call, then succeeded on unchanged retry. Correct handling under the standard adopted after a retraction earlier in this batch.
