# clone_test_run — DRIFT (one new behavioural gap; the rest is known systemic drift)
...
- **Run file:** `tests/live/runs/tm/clone_test_run.json`
- **Related:** `findings/get_test_run.md`, `findings/create_test_run.md` (the entity-wide schema drift)
...
The source carried an **accidental duplicate** row (`TC-54457` mapped twice, `latest_result_id` 2150387252 and 2150387283) created by `bulk_update_test_run_test_cases` and impossible to remove because its removal path no-ops.
...
- `list_test_runs` on `PR-2005`: **count 3 → 4**, with `TR-9063` present.
- `list_test_run_test_cases` on `TR-9063`: full row set returned on the very next call — **synchronous**, 200 (not the `202 {async:true}` its sibling returns).
- `list_test_run_test_cases` and the run header re-read on **`TR-9062`** afterwards: still exactly 5 rows with the identical five `latest_result_id` values, tags/assignee/run_state unchanged. **Cloning did not mutate the source.**
...
3. Fix the shared `test_run` response schema (tracked on `get_test_run`) — this capability comes right with it.
