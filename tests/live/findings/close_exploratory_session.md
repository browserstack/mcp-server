...
- **Session 780** (the clone): `actual_duration` was `null` before close (its long-standing broken value — the clone never inherited the derivation that produces a real number, per `get_exploratory_session.md` and `clone_exploratory_session_v1.md`). After close: still `null`. Closing did not populate it.
