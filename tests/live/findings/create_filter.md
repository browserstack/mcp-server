...
create_filter's actual behavior — a project-scoped, ungated, standalone saved-view object with its own `id`/`group_id` identity — fits the `filter` entity grouping well; nothing about it suggests it belongs elsewhere. This is a separate question from the previously-flagged placement of `list_entity_filter_details` under `project` instead of `filter`: that capability *resolves* a filter reference (a read/lookup keyed by entity+id) rather than operating on the filter object itself, so its placement is a distinct issue. `filter` as a grouping holds together fine for the CRUD family actually filed under it.
