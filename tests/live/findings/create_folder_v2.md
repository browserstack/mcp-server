...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/create_folder_v2.json`

## Headline: the v2 twin does NOT share the v1 defect

This capability was probed specifically because its v1 counterpart, `create_root_folder_v1`, was **published broken for its entire life** — declaring a flat `{name}` body while the API required `{folder:{name}}` (fixed in index v1.11).

**`create_folder_v2` does not have that defect.** Its declared parameter names — `name`, `description`, `parent_id`, `attachments` — are exactly what the surface accepts, and a folder was created successfully and verified. That is the primary result and it is good news.
...
A typed client that trusts the declaration will crash or mis-render on every newly created folder. This is the same family of defect as batch 1's `edit_project_v1`, where `projectVisibilityBanner` came back `null` from the write and an object from the very next read.

### The anomaly is specific to the create path

A third data point narrows this usefully. The sibling `move_folder_v2` probe returned the **same two fields on the same folder** (`764827`) and got real integers:
...
**This is not drift in `create_folder_v2`, and it should not be filed as such.** Evidence from across all three batches shows it is a consistent property of the invoke surface:

| capability | declared `json_path` | what happened |
| --- | --- | --- |
| `edit_project_v1` (batch 1) | `/project/{field}` | agent passed **flat**, tool nested under `project` server-side → 200 |
| `create_template_v2` (batch 1) | `none` | agent tried a `{template:{…}}` wrapper → same client-side rejection, `unknown body: template. accepted: …` |
| `create_folder_v2` (batch 3) | `/folder/{field}` | nested rejected client-side; **flat** → 200 |
...
- A second disposable folder, **764828** (`__mcp-probe-movetarget-…`), was also created under `764138` to serve as a move destination for the sibling `move_folder_v2` probe.
