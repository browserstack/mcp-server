...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/create_report_v2.json`
...
This is the same class of defect as `create_root_folder_v1` — a declared request shape the API won't accept — reached not through body nesting but through the required-field set. The response shape, by contrast, is flawless.
...
- `get_report_v2(PR-2005, SC-489)` → 200, byte-identical record.
- `list_reports_v2(PR-2005)` → `total_report_count: 1`, up from the **0 baseline** a sibling probe had established beforehand, with `SC-489` the sole row.
...
One thing to be aware of for future probes: **`frequency: "once"` is documented as a substitution for a stored null**, not a real schedule. A caller could read `"once"` as "this is a scheduled report that runs one time" when in fact no schedule exists. The sibling `get_report_v2` probe was asked to check that specifically.
