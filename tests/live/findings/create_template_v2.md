...
- **Run file:** `tests/live/runs/tm/create_template_v2.json`

## Summary

**The capability cannot be invoked successfully through its documented surface.** The contract declares `description_fields` and `properties_fields` optional; the live API rejects the create unless certain "non-removable System fields" are present — and those field ids are **not exposed by any documented read capability**, including `get_template_v2` against a known-good template of the same entity type.

This is the same class of defect as `create_root_folder_v1`: a published, structurally valid capability that no agent can actually use.
...
So the nesting is not the problem — unlike `create_root_folder_v1`, where it was.
