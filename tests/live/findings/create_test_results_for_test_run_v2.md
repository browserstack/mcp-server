# create_test_results_for_test_run_v2 — DRIFT (and the v1/v2 twins disagree on six axes)
...
- **Run file:** `tests/live/runs/tm/create_test_results_for_test_run_v2.json`
- **Related:** `findings/search_test_results_v1.md`, `findings/get_test_results_for_test_case_v2.md`
...
- **Before:** `get_test_results_for_test_case_v2` → `test_results: []`; the run row at `latest_status: "untested"`, `latest_result_id: 2150387254`.
...
- **Verify 1:** `get_test_results_for_test_case_v2` now returns **one row**, id 2150387456, `result_status.field_value: "Passed"` — an endpoint that had returned `[]` on every pairing ever tried, across two batches.
- **Verify 2:** `get_test_run_test_cases_v2` shows the row at `latest_status: "passed"`, `latest_result_id: 2150387456`.

## Finding 1 — the v1 and v2 twins disagree on six axes

The task asked specifically whether the v1 and v2 forms agree. **They do not**, and this is the most extensive same-entity disagreement the suite has found — worse than `get_` vs `list_custom_fields_v2`, and different in kind from the `test_plan` / `test_run` cases where a whole family was wrong the *same* way. Here the two forms are wrong **relative to each other**:
...
Enumerated via `get_test_results_for_test_case_v2`:
...
| `get_test_results_for_test_case_v2` → `result_status.field_value` | display case — `"Passed"` |
| `get_test_run_test_cases_v2` → `latest_status` | **lowercase** — `"passed"` |
| `search_test_results_v1` → flat `status` | **lowercase** internal token, and `null` in 29 of 30 rows |
...
2. **Fix `result_status`'s required-siblings declaration** on `get_test_results_for_test_case_v2` — only `field_value` is returned.
