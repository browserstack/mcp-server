# create_test_run — DRIFT (including a true `create_root_folder`-class defect)
...
- **Run file:** `tests/live/runs/tm/create_test_run.json`
- **Related:** `findings/create_test_plan.md` (the wrapper-guidance class), `findings/error-envelopes-systemic.md`
...
## Drift 1 — the declared `test_cases` shape is wrong (the create_root_folder class)
...
That is precisely the `create_root_folder` defect — a published capability whose declared request shape cannot work — and it is the first true instance this suite has found. An agent following the contract exactly cannot create a test run with test cases.
...
Worth recording as evidence in its own right. It means the wrapper guidance found on `create_test_plan`, `rename_folder`, `create_sub_test_plan` and others is not merely unhelpful — it describes the *post-mapping* HTTP body, and presenting that to a caller as a calling instruction is the mistake.
...
> **Answered (added 2026-09-18): it is entity-wide, and it is one fix.** The sibling `get_test_run` probe checked all 15 by name across **three** runs (`TR-9062`, `TR-9061`, `TR-9058`) and found **every one of them absent there too** — an identical gap list from an independent read on independent records. That rules out a create-response projection gap and points at a **shared, over-declared `test_run` response schema**. Correct the shared schema once and both capabilities come right together; do not patch per-endpoint. Same shape of problem as batch 4's `test_plan` family, where all five operations drifted in concert. See `findings/get_test_run.md`.
...
Consolidated in `findings/create_test_plan.md`; counted here only as another instance.

## No numeric run id exists anywhere on the v2 surface

Confirmed across three capabilities — the create response, `list_test_runs`, and `list_test_run_test_cases` all omit any numeric/uuid run id, and `list_test_runs`'s own guidance admits its serializer emits none.

**One serializer produces malformed URLs because of it.** The sibling `list_test_run_test_cases` probe found each row's `urls.self` comes back with an **empty path segment where the run id belongs**:
...
**Scoped by direct check (2026-09-18):** this is **specific to `list_test_run_test_cases`**, not general. The orchestrator called `get_test_run` directly and its embedded `test_cases[].urls.self` values are well-formed, with the identifier populated — `.../test-runs/TR-9062/1975349/2150387254`. So that endpoint interpolates `TR-NNNN` where the other leaves a hole. A localised bug in one serializer, worth fixing there.

**Consequence, stated accurately:** v1 run capabilities need the numeric id, and the v2 surface does not expose it — **but they are still reachable.** The `get_test_run_progress` probe resolved `TR-9062` → `17575382` via **`list_test_runs_by_integer_id`**, so a v1 listing call bridges the gap. (It also found that route accepts the `TR-NNNN` form directly, making the bridge unnecessary in that case.) The defect is the missing id on v2 and the misleading guidance about it — **not** an unreachable v1 surface, which is how this finding originally overstated it. The `get_test_run_progress` probe was asked to test exactly this — if the id cannot be resolved through any capability in the index, a whole v1 surface is unreachable from v2-created objects. (For reference, batch 1's `TR-9061` pairs with numeric `17574995`, so the ids do exist; the question is whether they are *obtainable*.)

## Verified against storage

`list_test_runs` on `PR-2005` before and after: **2 runs → 3**, with `TR-9062` present. `list_test_run_test_cases` confirmed all four requested cases actually landed, synchronously — no `202`/build-pending state was observed. Not trusted from the echo. Global search was not used (≥6-day indexing lag).
...
2. **Reconcile the 15 missing fields** — pending the `get_test_run` result, most likely by removing them from a shared `test_run` response schema that over-declares. Do not fix per-endpoint until that comparison is in.
