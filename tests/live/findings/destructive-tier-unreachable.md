...
Capabilities probed: `delete_custom_field_dataset_option_v2`, `delete_custom_field_dataset_v2`,
`delete_custom_field_definition_v2`, `delete_dataset`, `delete_filter`,
`delete_folder_and_contents`, `undo_remove_folder`, `delete_report_schedule`,
`delete_shared_step_v1`, `delete_test_plan_v1`, `delete_exploratory_session`,
`delete_exploratory_session_log`, `bulk_delete_test_cases`, `delete_v1_test_run`,
`bulk_delete_test_results_v1`, `delete_test_result_v1`.
...
- `undo_remove_folder` → `{"ok":false,"error":"undo_remove_folder is a destructive operation and is not available through this surface"}`
- `delete_test_result_v1` → `{"ok":false,"error":"delete_test_result_v1 is a destructive operation and is not available through this surface"}`
...
`delete_custom_field_definition_v2`, `delete_custom_field_dataset_v2`, the last two
...
(e.g. `delete_v1_test_run`'s guidance says "Confirm with the user before calling," and
...
- **Custom field `106131`** and its **datasets `31319`/`31320`** — delete paths for these
  *are* present in the published profile (`delete_custom_field_definition_v2`,
  `delete_custom_field_dataset_v2`, both directly probed above) but are unconditionally
