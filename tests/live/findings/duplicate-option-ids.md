...
in the lower block, yet `get_project_id_custom_fields_v2_v1` returned an **upper-block** id for
case_type "Other". A dedicated hunt through `get_test_runs_form_fields_v1` for a discriminator
...
`create_custom_field_definition_v2` minted field **106131** with options **146665 / 146666 /
146667**. Read back through `get_project_id_custom_fields_v2_v1` — the same capability that
...
Separately, `create_test_case_v1` was given the **upper** block deliberately — priority 318618,
