# `get_duplicate_source_test_case` — two reads disagree about whether the same duplicate exists

- **Product:** tm · **Capability:** `get_duplicate_source_test_case`
...
The route resolves only pairs in **merged** state. `list_duplicates` only ever surfaces pairs
...
(`get_dedupe_status` → `{status: "NOT_ENABLED", message: "No dedupe job found for this
...
- `findings/dedupe-status-disagreement.md` territory: `list_duplicates` reports
  `last_scan_time 2026-04-02` with 10 groups for the same project where `get_dedupe_status`
