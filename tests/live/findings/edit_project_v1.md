...
- **Probed:** 2026-09-17
- **Run file:** `tests/live/runs/tm/edit_project_v1.json`
...
| declared field | on this endpoint's 200 | on a plain `get_project_by_integer_id_v1` read |
| --- | --- | --- |
...
The subagent proposed that the absences reflect an account-level condition — project-level access control likely disabled on this preprod account. **That explanation holds for `access_type`**, which is missing from a plain read too. **It does not hold for `user_role` and `permissions`**, which the same subagent observed as *present* on `get_project_by_integer_id_v1` for this very project and *absent* here. That is an endpoint-specific gap independent of account configuration.
...
3. Change verified by fresh read, then the original description restored by a second `edit_project_v1` call, then the restore independently confirmed by a third read.
