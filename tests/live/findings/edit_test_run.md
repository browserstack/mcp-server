...
Both re-reads (`get_test_cases_for_v1_test_run`) confirm the run's case list becomes exactly the set sent — not the union of the existing set and the new ids. **This is a set/replace operation, not an add.**
...
This is exactly the trap that made `bulk_assign_test_cases_to_test_run` look like the tool for "add a case to a run" in the first place (it isn't — it only reassigns rows already present). `edit_test_run`'s own docs point an agent at the same mistake from a different direction.
...
Reproduced twice in this probe, independently confirmed via two different read capabilities each time (`get_test_run_by_integer_id_v1` and `get_test_run_cases_v1`):
...
`findings/test-run-writes-untrustworthy-systemic.md` already documented that `update_test_run_v2` and `replace_test_run_v2` return response bodies that disagree with storage for fields the caller didn't send in that call. This probe reproduces the same failure mode on **`edit_test_run` (v1)**:
...
`create_test_run_v1`'s probe isolated `project_id`-as-integer, `is_dynamic`-as-integer, and `overall_progress`-as-null to `get_test_runs_v1`'s list serializer, concluding the write path was consistently correct. Checking the same three fields on `edit_test_run`'s response:
...
Two of three are fine, but `project_id` is wrong here too. This means the earlier "one broken reader, write path is consistently right" conclusion does not fully hold: `project_id`-as-integer is shared by **both** `get_test_runs_v1` (list) and `edit_test_run` (write), while `create_test_run_v1` (a different write) gets it right. The bug looks tied to whichever serializer code path list and edit share, not to reads-vs-writes generally.
...
- `type` and `self_ui_link`: declared on this endpoint's own schema, never present in any of 4 edit responses taken. Consistent with `create_test_run_v1`'s finding — looks like a vestigial declared field across the whole `test_run` surface.
- `observability_url`: present and populated on every edit response. This is *better* than `create_test_run_v1`, where the same field is declared but always absent from the create response despite being computed and visible on an immediate read.
...
So `edit_test_run`'s serializer keeps two promises that `create_test_run_v1`'s serializer breaks, for the same entity — the two write paths are not uniformly wrong or uniformly right; each has its own gaps.

## What this unblocks

Despite the drift, the underlying goal succeeded: TC-54485 (1985019) and TC-54486 (1985020) are now mapped into TR-9110, with `mapping_id` **2150424052** (TC-54485) and **2150424053** (TC-54486) respectively, resolved via `get_test_cases_for_v1_test_run`. This gives `create_test_result_for_test_case`, `create_step_result_v1` and `update_latest_test_result_for_test_case` a probe-owned case to write results against, without using TC-54458 (the evidence case). **Any probe of those three capabilities must send the full existing case-id union if it ever calls `edit_test_run` itself, per Finding 1 — don't repeat the detachment.**
