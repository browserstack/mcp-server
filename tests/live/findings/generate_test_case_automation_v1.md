# generate_test_case_automation_v1 — BLOCKED
...
- **Run file:** `tests/live/runs/tm/generate_test_case_automation_v1.json`
...
This is the same evidentiary standard applied to `test_case_results_v1` (3 attempts, varied `issue_type` and query params) and `verify_test_case_tags_v1` (5 attempts, including the documented-valid empty array) before either was called BLOCKED.
...
Worth recording independently of the crash: **no companion status capability exists.** A search across automation-status, lcnc, build-link and related surfaces found nothing. The only indirect signal is re-reading the case via `get_test_case_v2`, which exposes `automation_status` but has **no field at all** for the `lcnc_link` / build metadata the guidance describes.
...
| --- | --- | --- |
| `test_case_results_v1` | `/api/v1/integrations/{issue_type}/…` | bare 500, every input |
| `verify_test_case_tags_v1` | `/api/v1/projects/{id}/test-cases/tags/verify` | bare 500, every input |
| `generate_test_case_automation_v1` | v1 | 422 + leaked Ruby error, every input |

(The suite's fourth BLOCKED, `create_template_v2`, is a v2 route and fails on *validation* rather than a crash — a different mechanism.)
