...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/get_custom_field_v2.json`
- **Related:** `findings/list_custom_fields_v2.md` — the two contracts contradict each other on `id`
...
`list_custom_fields_v2` (`p=1, count=300`) returned the whole 30-field workspace on one page. Two ids were chosen to cover shape variation:
...
`placeholder` is **the same undeclared duplicate that `list_custom_fields_v2` also returns**. Two capabilities exhibiting it points at a shared serializer, so it should be fixed there rather than twice.
...
## Internal contradiction with `list_custom_fields_v2`
...
| `list_custom_fields_v2` | string, form `cf-123` ❌ contradicted by reality |

This entry is the correct one. The `list` sibling appears to have inherited the stale published spec — see that finding for the full diff, including a `field_type` enum that is likewise wrong there and right in `create_custom_field_v2`.
...
2. **Resolve `placeholder`** — declare it, or drop it from the serializer as a duplicate of `place_holder_text`. Coordinate with `list_custom_fields_v2`, which returns it too.
