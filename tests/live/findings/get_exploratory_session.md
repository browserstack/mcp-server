...
`folder_id` is in this capability's declared return list. It is absent (key missing, not null) from the response on both 779 and 780. This is the same pattern already confirmed on `create_exploratory_session` and `clone_exploratory_session` for this entity (see `clone_exploratory_session.md`) — now also confirmed on the plain read.

## `project_id` type: this read returns integer; write paths return string

`project_id` is undeclared on this capability but present in the response as an **integer** (`379335744`) on both 779 and 780. The sibling `clone_exploratory_session` finding reported `project_id` as a **string** (`"379335744"`) on its own 201 response. Together these confirm the write serializers (create, clone) are the inconsistent side — the read is consistent with itself across both sessions probed here.
