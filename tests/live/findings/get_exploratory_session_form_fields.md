# get_exploratory_session_form_fields — DRIFT
...
- **Run file:** `tests/live/runs/tm/get_exploratory_session_form_fields.json`
...
| --- | --- | --- | --- |
| `test_case_results_v1` (batch 1) | **tracker-scoped** (`/integrations/{issue_type}/…`) | `{success:false, error:{code,message}}` | bare `{"status":500,"error":"Internal Server Error"}` |
| `verify_test_case_tags_v1` (batch 3) | ordinary project-scoped | `{success:false, error:{code,message}}` | bare `{"status":500,"error":"Internal Server Error"}` |
