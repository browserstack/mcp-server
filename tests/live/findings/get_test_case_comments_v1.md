# get_test_case_comments_v1 — DRIFT (guidance contradicts the response)
...
- **Run file:** `tests/live/runs/tm/get_test_case_comments_v1.json`
- **Primary write-up of the shared defect:** `findings/add_test_case_comment_v1.md`
...
Both descriptions apply to a **run-scoped** comment, not to this capability. The full analysis of the copy-paste root cause is in `findings/add_test_case_comment_v1.md`; what matters here is that the **read endpoint independently reproduces it**, so it is a property of the stored comment rather than an artifact of the write response.
...
Both fixes belong to the shared comment-entity defect — see `findings/add_test_case_comment_v1.md`:
