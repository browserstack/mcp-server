# get_test_case_histories_v2 — DRIFT (low severity)
...
- **Run file:** `tests/live/runs/tm/get_test_case_histories_v2.json`
...
| --- | --- | --- |
| V1 create | baseline name, `description: null`, priority Medium, tags `[mcp-probe]` | the `create_test_case_v2` probe (batch 5) |
| V2 update | `modified_fields: ["folder"]`, 764138 → 764828 | the `move_test_case_v2` probe |
| V3 update | 5 fields: description, priority, name, step result, tags | the `update_test_case_v2` probe's edit |
...
**It is not.** On the `test_case` side, `TC-54458`'s history `created_at` values are strictly increasing, real, and **byte-identical across three endpoints** (this v2 listing, `get_test_case_histories_v1`, and `get_test_case_history_v1` for the same rows). Change-tracking works correctly here.
