# get_test_plan_test_runs_v2 — DRIFT
...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/get_test_plan_test_runs_v2.json`
...
2. **`update_test_plan_v2` cannot link runs.** Its own guidance states a `test_runs` list sent there is **silently discarded**. So the probe used **`update_test_run_v2`** (mode `write`, not destructive) to set `test_plan_id: "TP-1616"` on `TR-9061` — the `__scratch__` run, not the readonly `TR-9058`.
3. **A fifth instance of the flat-vs-wrapped body trap.** That attach call had to be sent flat as `{"test_plan_id":"TP-1616"}`; a `test_run`-wrapped body — which `update_test_run_v2`'s own `json_path` implies — was rejected client-side: `unknown body: test_run. accepted: ...`. See `findings/create_test_plan_v2.md` for the systemic version of this.
...
| --- | --- |
| `get_test_plan_test_runs_v2` (this one) | integer `0` |
| `update_test_run_v2` (the attach call) | boolean `false` |
...
This is the third instance in the suite of two surfaces disagreeing about one value — after `create_folder_v2`'s `cases_count` (`null` on create, `0` everywhere else) and `edit_project_v1`'s `projectVisibilityBanner` (`null` from the write, an object from the read). Worth treating as a recurring class: **the index documents one shape per field, but serializers differ per endpoint.**
...
3. **Reconcile `is_dynamic`'s type** between this capability and `update_test_run_v2` — ideally in the API, since documenting two types for one field helps nobody. Worth checking whether other `filter_test_cases` sub-fields diverge the same way.
4. **Record that `update_test_plan_v2` silently discards `test_runs`** more prominently, and point callers at `update_test_run_v2` for linking. The current arrangement means the obvious capability for the job quietly does nothing.
