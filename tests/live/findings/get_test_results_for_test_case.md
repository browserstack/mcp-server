# get_test_results_for_test_case — DRIFT
...
- **Run file:** `tests/live/runs/tm/get_test_results_for_test_case.json`
- **Related:** `findings/get_test_results_for_test_case_v2.md` (the v2 sibling this probe was asked to compare against)
...
`get_test_results_for_test_case` (v1) and `get_test_results_for_test_case_v2` are semantically near-identical — same entity, same case-in-run scoping — and a routing eval treated them as a genuine tie because neither is marked preferred or deprecated in the index. Reading both contracts directly:
