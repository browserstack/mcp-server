# get_test_run — DRIFT (and a reproducible 500 on a declared query param)
...
- **Run file:** `tests/live/runs/tm/get_test_run.json`
- **Related:** `findings/create_test_run.md`, `findings/error-envelopes-systemic.md`
...
Every field the sibling `create_test_run` probe found absent is **also absent here**, across all three runs:
...
This compounds the gap noted in `findings/create_test_run.md`: v1 run capabilities need a numeric id, and the one place the index tells you to look for it is empty.

**But the v1 surface is not unreachable.** The sibling `get_test_run_progress` probe resolved `TR-9062` → `17575382` via **`list_test_runs_by_integer_id`**, and separately found that route accepts the `TR-NNNN` form directly anyway. So the practical impact is a misleading pointer and an extra bridging call, not a dead end — worth stating precisely, because the reverse would be a much more serious claim.
...
This also explains batch 4's `list_test_runs` observation that `overall_progress` was "omitted when nothing executed" — the field is sparse by nature, so an all-zero run yields an empty or absent object. The contract's fixed-seven-keys model does not describe this API.
...
>
> **What happened:** the orchestrator re-tested it directly. `minify=true` returned **200**. A sibling probe (`get_test_run_progress`) had meanwhile hit **three consecutive null-body 500s** on unrelated, already-trusted calls and correctly attributed them to preprod instability. So the original 2/2 was a flakiness burst that coincided with the `minify=true` calls, not a parameter-triggered crash. **A 2-of-2 reproduction is not sufficient evidence on an environment known to fail in bursts** — that is the lesson worth keeping.
...
`filter_test_cases.is_dynamic` came back **boolean** (`false`) on all three runs. Combined with `list_test_runs` (boolean) and `update_test_run` (boolean), that is **three capabilities returning boolean against one returning integer**.

`list_test_plan_test_runs` is conclusively the outlier, so the fix belongs there rather than in the contract. This closes a question open since batch 4.
...
7. **Fix `list_test_plan_test_runs`'s `is_dynamic`** to boolean, matching the other three.
