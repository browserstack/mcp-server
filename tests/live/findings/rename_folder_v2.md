# rename_folder_v2 — DRIFT (guidance falsified)
...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/rename_folder_v2.json`
- **Related:** `findings/create_folder_v2.md`, `findings/unlink_test_run_v1.md` (same verdict basis)

> **Verdict re-classified by the orchestrator** from the probing subagent's `PASS` to `DRIFT`. Its evidence is accepted unchanged; only the label is corrected. Batch 1's `unlink_test_run_v1` was called DRIFT because its *guidance* was wrong despite a schema-conformant success response — the same reasoning applies here, and labelling this PASS would contradict that precedent.
...
`cases_count` and `sub_folders_count` both came back as real integers (`0`, `0`) — matching `move_folder_v2`, **not** `create_folder_v2`'s `null`.

That completes the picture across four surfaces for the same empty folder `764827`:

| surface | `cases_count` / `sub_folders_count` |
| --- | --- |
| `create_folder_v2` | **`null`** ← outlier |
| `move_folder_v2` | `0` |
| `rename_folder_v2` | `0` |
| `list_folder_contents` | `0` |

The null is specific to the **create** path, which localises that fix to one serializer. See `findings/create_folder_v2.md`.
...
2. **Audit the rest of the folder family for the same copy-paste.** The line almost certainly came from `create_root_folder_v1`; anywhere else it was pasted, it will be equally wrong on a v2 route.
