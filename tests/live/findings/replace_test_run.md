# replace_test_run — DRIFT (the write's own response misreports what it wrote)
...
- **Run file:** `tests/live/runs/tm/replace_test_run.json`
- **Related:** `findings/assign_test_run_test_cases.md`, `findings/bulk_update_test_run_test_cases.md`, `findings/get_test_run.md`
...
A deliberately partial body (`{"description": "...", "tags": ["replace-probe-tag"]}`) returned 200. Comparing that response against two independent `get_test_run` re-reads (one after a delay):
...
| --- | --- |
| `replace_test_run` | **preserves** all 5 rows, duplicate included |
| `clone_test_run` | **dedupes** to 4, re-deriving the case list |
| `bulk_update_test_run_test_cases` (remove) | **no-ops** — cannot remove rows at all |
...
An undeclared **`issues` array** (`[]`) is returned by both this write **and** `get_test_run`. Since it appears on two capabilities it looks entity-wide rather than endpoint-specific, and it was not in the previously catalogued list — adding it to the shared-schema fix.

## Known entity-wide drift, recorded not re-litigated

The 15 declared-but-absent fields and the undeclared `filter_test_cases` / `urls` / `links.test_cases` all appeared exactly as catalogued on `create_test_run` and `get_test_run`; `assignee` again a plain string despite being declared an object; `overall_progress` again a sparse capitalised map. Fix once at the shared schema — see `findings/get_test_run.md`.
...
| --- | --- | --- |
| `assign_test_run_test_cases` | `200 {success:true}` | nothing happened |
| `bulk_update_test_run_test_cases` (remove) | `202 {success:true, async:true}` | nothing happened |
| `bulk_update_test_run_test_cases` (add) | 200 | happened *twice* — duplicated a row |
| **`replace_test_run`** | 200 | happened, but **the body misreports the result** |
