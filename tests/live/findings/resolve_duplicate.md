# `resolve_duplicate`: cannot be exercised — dedupe has never run for the only pool project

## Verdict: UNVERIFIED (prerequisite unavailable, not a payload or index defect)

`resolve_duplicate` is a `write`-mode capability on project 379335744 (PR-2005), live
...
- `get_dedupe_status` (probed separately, `tests/live/runs/tm/get_dedupe_status.json`,
...
- `list_duplicates`, invoked independently both by this probe and by a sibling
  (`tests/live/runs/tm/list_duplicates.json`, verdict UNVERIFIED), returns
  ...
  status read. Combined with `get_dedupe_status`, the ambiguity resolves to "feature
  never ran," not "clean project."
- `get_duplicate_source_test_case` (sibling probe,
  `tests/live/runs/tm/get_duplicate_source_test_case.json`, verdict UNVERIFIED) hit the
...
- This probe adds the missing middle piece: **`resolve_duplicate` cannot be the write
  ...
  `list_duplicates`'s own guidance — "there is no way to create a suggestion or trigger
  a scan through this API." Nothing reachable through `invokeCapability` can produce a
  duplicate pair from scratch.

One diagnostic call was made: `resolve_duplicate` with a deliberately out-of-range
...
`describeCapability`'s own guidance for `resolve_duplicate` states that `merge_action:
...
## `resolve_duplicate` vs `discard_duplicate`
