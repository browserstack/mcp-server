...
- **Run file:** `tests/live/runs/tm/search_test_results_v1.json`
...
`result_status.field_value` uses display case (`"Passed"`, `"Failed"`, `"Blocked"`) and **matches** what the write sibling `create_test_results_for_test_run_v2` documents (`status: "Passed"/"Failed"/"Untested"`, configured display-case names). So read and write agree on the *nested* field — good.
...
A caller doing `attachments.length` or `attachments.map(...)` gets a TypeError on every row, not an empty result. This is the same defect shape as `create_test_case_v2`'s omitted `attachments`, now on a different entity.
