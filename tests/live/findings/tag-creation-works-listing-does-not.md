...
- **Product:** tm · **Capabilities:** `list_test_case_tags`, plus the ~25 write capabilities that accept `tags`
...
new name to something — about 25 capabilities take a `tags` array, and `create_test_case` says
...
workflow sheet maps it to `verify_test_case_tags` — a *check whether these names are in use*
...
## The real defect: `list_test_case_tags` under-reports and omits new tags
...
immediately. `list_tags` is separately dead (500 on all five `entity_type` values, three
accounts).
...
Why `list_test_case_tags` reports a count that disagrees with its own pages, and why a tag
