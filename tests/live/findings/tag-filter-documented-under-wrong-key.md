...
1. **The plain listings have no filters at all.** `get_test_cases_v1` and
   `list_folder_test_cases_v1` declare no tag, status or any other filter parameter. Filtering
   lives on `search_project_entities` / `search_project_entities_v2`. A caller reaching for
...
`findings/status-by-name-silently-nulls.md` — resolve ids with `get_system_field_values_v1`
first, where the id is in `value` and custom options are the ones with no `colour`.

## `search_project_entities_v2` was already correct
