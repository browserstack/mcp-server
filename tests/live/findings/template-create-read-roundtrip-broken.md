# `get_template_v2` drops the two fields `create_template_v2` refuses to work without

- **Product:** tm · **Capabilities:** `create_template_v2`, `get_template_v2`
...
`create_template_v2` rejects any body whose field list omits system fields **17 and 18** (the
Review & Approve pair):
...
`get_template_v2` never returns those two fields. On **every** template in the workspace the
...
Return the non-removable system fields from `get_template_v2` — that alone closes the loop. Failing
