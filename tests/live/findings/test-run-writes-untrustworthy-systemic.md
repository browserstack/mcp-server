...
- **Per-capability detail:** `assign_test_run_test_cases_v2.md`, `bulk_update_test_run_test_cases_v2.md`, `replace_test_run_v2.md`, `update_test_run_v2.md`
...
| --- | --- | --- |
| `assign_test_run_test_cases_v2` | `200 {success:true}` | **nothing** — rows unchanged across 2 re-reads |
| `bulk_update_test_run_test_cases_v2` — remove | `202 {success:true, async:true, unique_id}` | **nothing** — unchanged across 3 re-reads over 45 s |
| `bulk_update_test_run_test_cases_v2` — add | `200` | **happened twice** — duplicated an existing mapping, 4 rows → 5 |
| `replace_test_run_v2` | `200` | happened, but the **body misreported the result** |
| `update_test_run_v2` | `200` (×2 writes) | happened, but the **body misreported the result** — same fields |

The two clean capabilities were `clone_test_run_v2` (verified: run count 3 → 4, source untouched) and `export_test_run_csv_v1` (verified: run byte-identical before and after).
...
`replace_test_run_v2` and `update_test_run_v2` both return bodies that disagree with storage, **on the same three fields**:
...
`update_test_run_v2` reproduced this across **two independent writes**, and a control read of untouched fixture `TR-9058` stayed static and self-consistent throughout — so it is not preprod noise. Two capabilities spanning PATCH and POST on the same route family makes it entity-wide.
