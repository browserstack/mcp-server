...
- **Probed:** 2026-09-18
- **Run file:** `tests/live/runs/tm/update_test_plan.json`
- **Related:** `findings/create_test_plan.md`, `findings/list_test_plan_test_runs.md`
...
Rejected **client-side**. Flat succeeded. So the recommended form fails and the "also accepted" alternative is the only one that works — identical to `create_test_plan`, and the **seventh** instance in this suite (`create_folder`, `rename_folder`, `create_template`, `create_test_plan`, `create_sub_test_plan`, `update_test_run`, and this one).
...
That is a deliberately-documented trap, and a sibling probe relied on it (routing around via `update_test_run` instead, which worked). **But it does not happen.** `test_runs` is not a declared body field on this capability at all, and sending it flat was rejected **client-side before any HTTP request**:
...
- **The update works and persists.** Name, description and tags were changed, then confirmed by an independent `get_test_plan` read — not trusted from the response echo.
- **Sparse-update semantics hold.** Omitted fields kept their stored values.
- **Run linkage was untouched.** `list_test_plan_test_runs` still returned exactly `TR-9061`, nothing added or removed — so whatever the guidance says about discarding, the update genuinely does not disturb linked runs.
- **Response shape matched the declared `returns` list**, with no undeclared fields, and with `parent_plan_id` correctly absent (top-level plan). The flat `returns` array overclaims it unconditionally while the nested schema correctly scopes it to sub-plans — the same known pattern as the sibling capabilities, cross-referenced in `findings/get_test_plan.md` rather than re-filed here.
...
1. **Fix the wrapper guidance** — recommend flat. Part of the suite-wide wrapper audit (see `findings/create_test_plan.md`).
...
3. **Keep pointing callers at `update_test_run`** for linking a run to a plan — that part of the advice is correct and useful, and it is the only working route.
4. Fix the flat `returns` overclaim of `parent_plan_id` (tracked in the `get_test_plan` finding; same root cause).
