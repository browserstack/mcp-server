...
describe("the released artifact", () => {
  it("loads the product from its own top-level key", () => {
    // COUNTS ARE NOT PINNED. The index is regenerated by the export pipeline and grows
    // whenever a product onboards more endpoints — it went 173 -> 218 capabilities and
    // 19 -> 21 entities in three commits. A test that hardcodes the total fails on every
    // regeneration while telling you nothing about the code, so what is asserted here is
    // that the artifact LOADS and is internally consistent.
    expect(registry.productNames()).toEqual(["tm"]);
    const tm = registry.index.products.tm;
    expect(tm.capabilities.length).toBeGreaterThan(0);

    // Every capability is filed under an entity that exists, and every entity's
    // capability list resolves to a real record. Both directions, because a dangling ref
    // in either one hands a caller an identifier it cannot use.
    const entities = new Set(Object.keys(tm.entities));
    expect(tm.capabilities.filter((c) => !entities.has(c.entity))).toEqual([]);
    const names = new Set(tm.capabilities.map((c) => c.name));
    for (const [entity, doc] of Object.entries(tm.entities)) {
      for (const name of (doc as any).capabilities ?? []) {
        expect(names.has(name), `${entity} -> ${name}`).toBe(true);
      }
    }
  });
  ...
  it("publishes the page ceiling under the name the artifact uses", () => {
    ...
    // Bounded, not pinned. What matters is that a paginated capability CAN declare its
    // ceiling and that some do — the count moves with every export. Coverage is thin and
    // is a product-authoring gap, not a format one.
    expect(paged.filter((c) => c.max_page_size !== undefined).length).toBeGreaterThan(0);
  });
  ...
  it("refuses every destructive endpoint before binding", () => {
    ...
    // tm spells most deletes as `POST .../delete`, so this is populated by the path rule
    // as well as the method. Asserted as non-empty rather than counted: the number grows
    // with every onboarding batch, and what the test is for is that the classification
    // happens at all and that every one of them is refused before binding.
  ...
  it("marks the endpoints whose response the product never declared", () => {
    ...
    // A minority, and bounded rather than pinned: a product declaring response schemas
    // for everything would make this 0, which is the direction we want it to move.
    expect(discovered.length).toBeLessThan(
      registry.index.products.tm.capabilities.length / 2,
    );
  });

  it("carries the response and schema tables, stored once and referenced", () => {
    const tm = registry.index.products.tm;
    expect(Object.keys(tm.responses || {}).length).toBeGreaterThan(0);
    expect(Object.keys(tm.schemas || {}).length).toBeGreaterThan(0);
    // EVERY capability declares its responses — asserted as a ratio against the whole,
    // not a fixed number, so it keeps holding as the index grows. The payload stays small
    // because the bodies live in the tables and the capability only names them.
    expect(tm.capabilities.filter((c) => c.responses).length).toBe(
      tm.capabilities.length,
    );
  });
