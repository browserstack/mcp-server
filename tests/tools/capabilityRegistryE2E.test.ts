 ...
 * there is no ambiguity to detect and the gate is correct to stay silent. This branch ships
 * tm alone, so the second product comes from `fixtures/capability-multi/` — test data that
 * is never published (see that directory's README). The shipped tm index is used as-is so
...
for (const [from, name] of [
  ...
  [
    new URL("../fixtures/capability-multi/loadtesting.capability-index.json", import.meta.url),
