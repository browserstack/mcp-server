 ...
 *
 * tm is the shipped index; the second comes from `fixtures/capability-multi/`, which is
 * test data and is not published. See that directory's README — the cross-product gate is
 * shipped code, so it needs two products under test even though only one ships.
 */
...
const lt = load(
  "loadtesting.capability-index.json",
...
describe("routing between products", () => {
  ...
  it("says what each entity IS, not only what it answers to", () => {
    ...
    // Absent, not empty, where a product has not authored them. The second product carries
    // 9 entities and 0 descriptions, which has to read as "unwritten" rather than as a
    // build that produced nothing — the case of a product onboarded before its vocabulary
    // is written.
